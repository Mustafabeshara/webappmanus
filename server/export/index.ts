...
export function exportToExcel(options: ExportOptions): ExportResult {
  try {
    ...
    for (const row of data) {
      ...
      for (const col of columns) {
        const value = formatValue(row[col.key], col.format);
        const type = col.format === 'number' || col.format === 'currency' || col.format === 'percent'
          ? 'Number'
          : 'String';
...
export const EXPORT_CONFIGS = {
  tenders: {
    columns: [
      ...
      { key: 'estimatedValue', label: 'Estimated Value', format: 'currency' as const },
  ...
  customers: {
    columns: [
      ...
      { key: 'email', label: 'Email' },
      { key: 'phone', label: 'Phone' },
      { key: 'creditLimit', label: 'Credit Limit', format: 'currency' as const },
    ],
  },
};
